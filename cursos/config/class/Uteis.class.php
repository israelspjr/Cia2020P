...
class Uteis {
  ...
  //ENVIAR E-MAIL
  static function enviarEmail($assunto = "", $mensagem = "", $paraQuem = array(), $arquivos = array(), $copia = array(), $bcopia = array(), $emailMkt = 0, $from = '', $reply = '') {
...
//    $mailer -> SMTPSecure = 'ssl';
	...
	if ($from == '') {
		$mailer->setFrom(FROM, $paraQuem['email']);//$config[0]['email']);
			...
			<body style=\"border:0\">
				<table cellpadding=\"0\" cellspacing=\"2\" align=\"center\" class=\"principal\">
					<tbody>
						...
						<tr>
							<td class=\"rodape\">												
							<img alt=\"Companhia de Idiomas\" border=\"0\" src=\"".CAMINHO_IMG3.$config[0]['rodape']."\" width=\"650\"/></a>
							</td>
						</tr>
					</tbody>
				</table>
...
//IMPRIME CÓDIGO JS
  ...
  static function getWeeks($date, $rollover)
    {
        $cut = substr($date, 0, 8);
        $daylen = 86400;
