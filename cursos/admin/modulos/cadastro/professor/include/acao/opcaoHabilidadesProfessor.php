...
//$Habilidades->setIdHabilidad($idProfessor)
$HabilidadesProfessor->deleteHabilidadesProfessor(" idProfessor = ".$idProfessor);

$valoresHabilidades = $Habilidades->selectHabilidades(" WHERE inativo = 0 AND tipo = 0");
...
for ($row = 0; $row < count($valoresHabilidades, 0); $row++){
	...
	if($field==1){
		...
		$Habilidades->setDescricao($_POST['outros']);
		$valor = $Habilidades->addHabilidades();
		...
		$HabilidadesProfessor->setIdHabilidade($valor);
