...
<script>
	function aguardarCarregamento(){		
		if( $('#visualizar_contrato[status=esperando]').length > 0 ){
			...
			postForm('form_contrato', '<?php echo CAMINHO_CAD?>funcionario/include/acao/contrato.php?id=<?php echo $idContrato?>');
		}
	}
</script>
<div class="conteudo_nivel">
  ...
  <form id="form_uploadFile" method="post" enctype="multipart/form-data" action="<?php echo CAMINHO_CAD?>funcionario/include/acao/contrato.php" style="display:none">
  ...
  <fieldset>
    ...
    <form id="form_upload_contrato" method="post" enctype="multipart/form-data" action="<?php echo CAMINHO_CAD?>funcionario/include/acao/contrato.php" style="display:none;">
      <input type="hidden" id="acao" name="acao" value="file" />
      <input type="file" id="add_file" name="file" />
    ...
    <form id="form_contrato" class="validate" method="post" action="" onsubmit="return false" >
	 ...
	 if($idContrato != '' && is_numeric($idContrato)){
      ...
      <p>
      Enviar Email <input type="checkbox" name="enviarEmail" id="enviarEmail" value="1" />
      </p>
