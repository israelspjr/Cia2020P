...
}elseif( $_REQUEST['acao'] == "aulaInexistente"){
...
//CADASTRO DE REPOSIÇÃO MANUAL
	...
	//			echo $dataFechamento." ---".$dataAula;	
		...
		$arrayRetorno['ondeAtualizar'] = "#div_ff_geral";
		$arrayRetorno['pagina'] = CAMINHO_REL."grupo/include/form/folhaFrequencia.php?idFolhaFrequencia=".$idFolhaFrequencia."&naoAtualizarMais=1";	
...
//CADASTRO DE REPOSIÇÃO MANUAL
	...
	if($difHoras >= 0){
		...
		onclick=\"deletaRegistro('".CAMINHO_REL."grupo/include/acao/diaAulaFF.php', '$idDiaAulaFF_rep', '".CAMINHO_REL."grupo/include/form/folhaFrequencia.php?idFolhaFrequencia=$idFolhaFrequencia', '#div_ff_geral')\" />&nbsp;"
	...
	if($difHoras > 0){
	//VERIFICAR SE A DIFERENÇA NAS HORAS PARA LIBERAR OCORRENCIA		
...
//		Uteis::pr($oc);
		...
		$res = $OcorrenciaFF->selectOcorrenciaFFSelect("", $idOcorrenciaFF, " AND adminVe = 1 ");
